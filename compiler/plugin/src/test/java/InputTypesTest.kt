...
"""
    ...
    fun `Detects custom types as default input`() {
        ...
        assertContains(
            result.messages,
            "generating MyInterface#Do2(\"default_input\") with types: DEFAULT(CustomType) DEFAULT(Int)"
        )
    }
    ...
    fun `Detects binary input`() {
        ...
        assertContains(
            result.messages,
            "generating MyInterface#Do3(\"binary_input\") with types: BINARY(ByteReadChannel) DEFAULT(Int)"
        )
    }
    ...
    fun `Detects service input`() {
        ...
        assertContains(
            result.messages,
            "generating MyInterface#Do4(\"service_input\") with types: SERVICE(MyInterface) DEFAULT(Int)"
        )
    }
}
