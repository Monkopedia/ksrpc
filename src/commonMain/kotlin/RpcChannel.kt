...
interface RpcChannel : SuspendCloseable {
    ...
    suspend fun <I> callBinary(
        endpoint: String,
        inputSer: KSerializer<I>,
        input: I,
    ): ByteReadChannel

    suspend fun <O> callBinaryInput(
        endpoint: String,
        outputSer: KSerializer<O>,
...
private class SubserviceChannel(
    ...
    private suspend fun <I, O> call(
        ...
        return baseChannel.call(
            json.encodedEndpoint(target + serviceId), inputSer, outputSer,
            input
        )
    }
    ...
    private suspend fun <O> callBinaryInput(
        target: List<String>,
        outputSer: KSerializer<O>,
        input: ByteReadChannel,
    ): O {
        ...
        return baseChannel.callBinaryInput(
            json.encodedEndpoint(target + serviceId), outputSer,
            input
        )
    }
